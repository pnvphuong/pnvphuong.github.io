...
1. Add `$FRCN/lib/datasets/[yourDS].py`: reading/loading a part of the whole dataset
  ...
  1.1. Modify `self._classes` in the constructor function to fit your dataset.
  1.2. Be careful with the *extensions* of your image files. See `image_path_from_index` in `inria.py`.
  1.3. Write the function for parsing annotations. See `_load_inria_annotation` in `inria.py`.
  1.4. Do not forget to add `import` syntaxes in your own python file and other python files in the same directory.
