...
5. run install script
    ...
    note for Dogwood rc3 installation:   `OPENEDX_RELEASE=named-release/dogwood.rc` and run `sudo apt-get install libmysqlclient-dev -y` and `pip install --upgrade pip` before installing Dogwood to overcome the error of /tmp/pip-build-3CgUfy/MySQL-python (this bug will be fixed in the official release) and `cffi` installation
6. restart VM
