...
To change the _sender_ email address (which is _registration@example.com_ by default), we can search all files containing the word _registration@example.com_. By doing this on console, `sudo grep -rl "registration@example.com" /edx/ > ~/match.txt` . This command will search all files containing _registration@example.com_ and write into the file _match.txt_ in the home folder.
