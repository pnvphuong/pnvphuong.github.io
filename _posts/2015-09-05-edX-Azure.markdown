...
2. Go to the folder 
    ...
    Modify [files](https://github.com/edx/edx-platform/search?q=django.core.mail.backends.console.EmailBackend&type=Code) on your system: changing `django.core.mail.backends.console.EmailBackend` to _django.core.mail.backends.**smtp**.EmailBackend_
3. Add your gmail information in `cms/envs/common.py`
    ...
    As well as in `lms/envs/aws.py`
                
        EMAIL_HOST = ENV_TOKENS.get('EMAIL_HOST', 'smtp.gmail.com') # django default is localhost
        EMAIL_PORT = ENV_TOKENS.get('EMAIL_PORT', 587) # django default is 25
        EMAIL_USE_TLS = ENV_TOKENS.get('EMAIL_USE_TLS', True) # django default is False
        EMAIL_HOST_USER = AUTH_TOKENS.get('EMAIL_HOST_USER', 'youruser@yourdomain.com') # django default is ''
        EMAIL_HOST_PASSWORD = AUTH_TOKENS.get('EMAIL_HOST_PASSWORD', 'YourPassword') # django default is ''
...
4. Restart services (I tried `sudo su edxapp bash` before doing this but not sure if it is necessary or not)

* LMS/CMS `sudo /edx/bin/supervisorctl restart edxapp:`
* Workers `sudo /edx/bin/supervisorctl restart edxapp_worker:`

To change the _sender_ email address (which is _registration@example.com_ by default), we can search all files containing the word _registration@example.com_. By doing this on console,
...
The above information may not be correct. Found 4 important files for smtp:
...
Still couldn't make it send through gmail. But at least, it raised some error related to smtp from gmail (SMTPAuthenticationError: (534, '5.7.14 <https://accounts.google.com/ContinueSignIn?sarp=1&scc=1&plt=AKgnsbuQq\n5.7.14 -piC_VEVrfPseeYEDyBMGXr5rsu1OTVHwTEL-9c-6svnEg87xsaxIuU2lKQ1un-jpoiiGL\n5.7.14 DiRc_ZdWIAnHldZCyHXEt9PElvKawOTE490BKGaP4o-ZHhRFWEmbV3lSCDWvAkNjU4scti\n5.7.14 ZMX_tgpl5I_OqyPYNVL0YL9KXxA1gyGBM7yKcKWoMf98riNLm_TwJTPeQ7iGPM4E_sATpq\n5.7.14 Dyq0hfJs3kYsVCZels2NmS5GEghE> Please log in via your web browser and\n5.7.14 then try again.\n5.7.14  Learn more at\n5.7.14  https://support.google.com/mail/answer/78754 5sm1809000qky.21 - gsmtp')).
Note that we can use Java to send email from the gmail account successfully. So it may because of either Django setting on the server or Gmail account (but Gmail account worked with java already!)
